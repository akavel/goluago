...
var code_db = `-- testing debug library
...
-- TODO: condition above was disabled because we 'loadstring', not 'loadfile'
...
function f(a,b)
  ...
  assert(debug.setlocal(2, 4, "ma` + "\xe7" + `` + "\xe3" + `") == "B")
  x = debug.getinfo(2)
...
function g(...)
  ...
  local feijao
  local AAAA,B = "xuxu", "mam` + "\xe3" + `o"
  f(AAAA,B)
  assert(AAAA == "pera" and B == "ma` + "\xe7" + `` + "\xe3" + `")
  do
     local B = 13
